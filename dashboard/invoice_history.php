     ...
     <div class="col-sm-9">
      <button style="position: absolute; left: 0;top: 200px;background-color: white; border: 1px solid white;" onclick="printContent('print')">p</button>
       ...
       <div style="position: absolute; top: 70px; left: 10px; width: 98%; height: 94.7%;">
         <p style="padding-left: 420px; padding-top: 6px; font-size: 23px; font-family: copper black;">Invoice History</p><hr>
         <div  style="position: absolute; top: 63px; width: 100%; height: 96.6%;">
          <form action="invoice_history.php" method="post">
        ...
        <table id="example" class="display" cellspacing="0" width="100%" style="margin-top: 100px;">
         <thead>
            <tr style="background-color: #eee;border-bottom:1pt solid black;">
                ...
                <th style="text-align: center;">Submitted TK</th>
                <th style="text-align: center;">Owing TK</th>
         ...
         <tbody>
            <?php 
              ...
              if(isset($_POST['from_date'])){
                 ...
                 $customer_name = $_POST['customer_name'];
                  ...
                  for ($i=$from_date; $i<=$to_date; $i+=86400){
                     ...
                     if($customer_name!=""){
                        $query  = "SELECT * from invoice where shopcode='$codeshop' AND datee='$time' AND name='$customer_name'";
                     }else{
                        $query  = "SELECT * from invoice where shopcode='$codeshop' AND datee='$time'";
                     }
                     ...
                     while ($row= mysql_fetch_array($result)) {
                      ...
                      $bill = $row['bill_no'];
                      $total=$total+$amount;
                      $total_submit=$total_submit+$row['Submit_money'];
                      $total_owing=$total_owing+$row['owing_money'];
               ?>

              <tbody>
                <tr class="invoice_list">
                  <th style="text-align: center;"><?php echo $row['datee']; ?></th>
                  <th style="text-align: center;"><?php echo $bill; ?></th>
                  <th style="text-align: center;"><a href="invoice_memo2.php?id=<?= $row['bill_no']?>&&namee=<?= $row['name']?>&&date=<?= $row['datee']?>"><?php echo $row['name']; ?></a></th>
                  <th style="text-align: center;"><?php echo $row['address']; ?></th>
                  <th style="text-align: center;"><?php echo $row['Submit_money']; ?></th>
                  <th style="text-align: center;"><?php echo $row['owing_money']; ?></th>
                  <th style="text-align: center;"><?php echo $amount; ?></th>
                  <th style="text-align: center;"><a href="edit_invoice_money.php?bill=<?=$bill?>">edit</a></th>
                </tr>
              </tbody>
              ...
              <?php
                     ...
                     $query  = "SELECT * from invoice WHERE shopcode='$codeshop' ORDER BY bill_no DESC LIMIT 10";
         ...
         <tfoot>
            <tr style="background-color: #eee;border-top:1pt solid black;">
                ...
                <th style="text-align: center;">Customer Name</th>
                <th style="text-align: center;">Customer Address</th>
                <th style="text-align: center;">Total= <?php echo $total_submit; ?></th>
                <th style="text-align: center;">Total= <?php echo $total_owing; ?></th>
