...
<body>
  ...
  <div class="row content">
    ...
    <div class="main-body">
     <div class="col-sm-9">
      <button style="position: absolute; left: 0;top: 220px;background-color: white; border: 1px solid white;" onclick="printContent('print')">p</button>
        <script>
       ...
       <div style="position: absolute; top: 100px; left: 10px; width: 98%; height: 94.7%;">
         <p style="padding-left: 380px; padding-top: 6px; font-size: 23px; font-family: copper black;">Shop Product History</p><hr>
         <div  style="position: absolute; top: 63px; width: 100%; height: 96.6%;">
          <form action="shop_product_history.php" method="post">
           <div style="position: absolute; top: 10px; word-spacing: 50px;">
             ...
             Vendor-Name:<input type="text" name="vendor_name" list="vendor">
             <datalist id="vendor">
                <?php
            ...
            <?php 
              ...
              if(isset($_POST['from_date'])){
                 ...
                 $vendor_name = $_POST['vendor_name'];
                  ...
                  for ($i=$from_date; $i<=$to_date; $i+=86400){
                     ...
                     if($vendor_name!=""){
                        $query  = "SELECT * from shop_product where shopcode='$codeshop' AND datee='$time' AND name='$vendor_name'";
                     }else{
                        $query  = "SELECT * from shop_product where shopcode='$codeshop' AND datee='$time'"; 
                     }
                     $result = mysql_query($query) or die(mysql_error());
                     while ($row= mysql_fetch_array($result)) {
                      $amount=$row['amount'];
              ...
              <tbody>
                <tr class="shop_list">
                  ...
                  <th style="text-align: center;"><?php echo $row['shop_no']; ?></th>
                  <th style="text-align: center;"><a href="shop_product_memo2.php?id=<?= $row['shop_no']?>&&namee=<?= $row['name']?>&&date=<?= $row['datee']?>"><?php echo $row['name']; ?></a></th>
              ...
              <?php
                     ...
                     $query  = "SELECT * from shop_product WHERE shopcode='$codeshop' ORDER BY shop_no DESC LIMIT 10"; 
                     $result = mysql_query($query) or die(mysql_error());
                     while ($row= mysql_fetch_array($result)) {
                      $amount=$row['amount'];
                      $total=$total+$amount;
